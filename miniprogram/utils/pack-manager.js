...
class PackManager {
  ...
  async openPack(packId) {
    ...
    for (let i = 0; i < packInfo.quantity; i++) {
      ...
      // 检查是否为新卡（未收集且本次未抽到过）
      const isNewCard = !collectedCards.has(card.card_id) && !currentDrawnCards.has(card.card_id)
      console.log(`卡包管理器 - 抽到卡片: ${card.name}, 是否新卡: ${isNewCard}`)
      console.log(`卡包管理器 - 判断依据: 在收集列表中: ${collectedCards.has(card.card_id)}, 本次已抽到: ${currentDrawnCards.has(card.card_id)}`)
    ...
    if (result.newCards.length > 0 || result.totalStars > 0) {
      ...
      console.log(`卡包管理器 - 需要同步 ${result.newCards.length} 张新卡, ${result.totalStars} 颗星星`)
      ...
      try {
        ...
        if (result.newCards.length > 0) {
          ...
          for (const set of currentSets) {
            const setCards = albumManager.getSetCards(set.set_id)
            const isCompleted = setCards.every(card => collectedCards.includes(card.card_id))
            const isInCompletedList = syncManager.getCompletedSets().includes(set.set_id)
